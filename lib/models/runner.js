...
/**
* Builds the list of tests to send up
**/
var buildTestList = function(modules) {
  ...
  for(var i = 0; i < (modules || []).length; i++) {
    ...
    // flatten and add each of the tests
    if(modules[i].getKey &&
        modules[i].getCategory &&
          modules[i].getVersion) {

      // add it
      tests.push({

        key:        modules[i].getKey(),
        category:   modules[i].getCategory(),
        version:    modules[i].getVersion()
...
module.exports = exports = function() {
  ...
  **/
  try {
    ...
    if(vals.length > 0) {
      ...
      if(vals[0].length === null ||
          vals[0].length === undefined) {

        // set options
        options = _.extend({}, options, arguments[0] || {});

        // build the list of checks
        trackedTests  = buildTestList(vals.slice(1, vals.length));
        checks        = buildCheckList(vals.slice(1, vals.length));
        bootstraps     = buildBootstrapList(vals.slice(1, vals.length));
        ...
        if(options.bootstrap &&
          ...
          bootstraps.push(options.bootstrap)
        ...
        // build the list of checks
        trackedTests  = buildTestList(vals);
        checks        = buildCheckList(vals);
        bootstraps     = buildBootstrapList(vals);
  ...
  **/
  Runner.bootstrap = function(fn) {
    ...
    // bootstrap it
    async.each(bootstraps, function(bootstrapFunc, cb) {

      // run the bootstrap
      bootstrapFunc(options.logger || console, cb);
  ...
  *
  * Can be disabled using the 'reporting' flag, see readme at:
  * github.com/passmarked/passmarked
  **/
