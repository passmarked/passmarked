...
module.exports = exports = function(params, har, content) {
  ...
  * the idea is that the cache methods are overridden
  * to fetch from a real cache like Redis
  **/
  var cache = _.extend({}, options.cache || {});

  /**
  * Sets the start of a checkpoint
  **/
  Payload.start = function(key) {

    // set the start
    checkpoints[key] = new Date().getTime();
  ...
  * Ends the checkpoint
  **/
  Payload.end = function(key) {

    // check if the end was set
    if(!checkpoints[key]) return;

    // get the start
    var started = checkpoints[key];
    ...
    // check if in development and bigger than 100
    if(process.env.NODE_ENV != 'production' &&
        delta > 50) {

      // just output
      Payload.warn(key + ' took ' + Math.round(delta) + 'ms');

    } else if(process.env.NODE_ENV != 'production') {

      // just output
      Payload.debug(key + ' took ' + Math.round(delta) + 'ms');
  ...
  **/
  Payload.warn = function(tag, message) {

    // add a alias
    Payload.warning(tag, message);
      ...
      if(type) {
        ...
        if(occurrence) {
          ...
          if((params.length && params.length === -1) ||
            ...
            // push to array
            rules[ params.key ].occurrences.push( occurrenceTest );
