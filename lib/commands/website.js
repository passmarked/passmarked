...
module.exports = exports = function(payload, fn) {
  ...
  config.build(function(){
    ...
    if(S(websiteid).isEmpty() === true || S(websiteid).isNumeric() === false) {

      // output a error explaining what's happening
      payload.error('***************\n\nNumerical Website ID must be provided\n\n***************');
    ...
    api.websites(token, function(err, response) {
      ...
      var item = _.find(response.items || [], function(website) { return 1 * website.id == 1 * websiteid });
