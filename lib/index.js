...
Client.commands = {
  ...
  tokenGet:     require('./commands/token.get'),
...
Client.handleOutput = function(err) {
  ...
  **/
  ...
  if(args.format == 'json') {
    ...
    // output debugging
    process.stdout.write(payload.getJSON() + '\n', function() {
    ...
    // output the XML
    process.stdout.write(payload.getXML() + '\n', function() {
    ...
    process.stdout.write(payload.getText() + '\n', function() {
...
Client.executeCommand = function(path) {
  ...
  if(cleanedPath == 'version') {
  ...
  } else if(cleanedPath.indexOf('token') == 0 && cleanedPath.split(' ').length == 2) {

    // route to token
    Client.commands.tokenSet(payload, Client.handleOutput);

  } else if(cleanedPath == 'token') {

    // route to token
    Client.commands.tokenGet(payload, Client.handleOutput);
    ...
    async.each(urls, function(givenUrl, cb) {
      ...
      if(cleanedUrls.length == 0) {
        ...
        // show the error
        Log.error('Expecting urls to by supplied');
